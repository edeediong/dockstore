...
@NamedQueries({
    @NamedQuery(name = "Entry.getGenericEntryById", query = "SELECT e from Entry e WHERE :id = e.id"),
        ...
        @NamedQuery(name = "io.dockstore.webservice.core.Entry.findDescriptorTypes", query = "SELECT distinct new io.dockstore.webservice.core.database.FileTypeDTO(wv.id, sf.type) from WorkflowVersion wv join wv.sourceFiles sf where wv.id in :ids"),
        @NamedQuery(name = "io.dockstore.webservice.core.Entry.getImagesForVersions", query = "SELECT new io.dockstore.webservice.core.database.ImageDTO(i, wv.id) from WorkflowVersion wv join wv.images i where wv.id in :ids"),
